#' Create a new RosterResponse
#'
#' @description
#' RosterResponse Class
#'
#' @docType class
#' @title RosterResponse
#' @description RosterResponse Class
...
#' @field data  list(\link{RosterPublic})
#' @importFrom R6 R6Class
#' @importFrom jsonlite fromJSON toJSON
#' @export
RosterResponse <- R6::R6Class(
  "RosterResponse",
  public = list(
    `count` = NULL,
    `total` = NULL,
    ...
    #' @description
    #' Initialize a new RosterResponse class.
    #'
    #' @param count count
    #' @param total total
    #' @param limit limit
    ...
    #'
    #' @return RosterResponse as a base R list.
    #' @examples
    #' # convert array of RosterResponse (x) to a data frame
    #' \dontrun{
    #' library(purrr)
    #' library(tibble)
    ...
    #' @description
    #' Convert RosterResponse to a base R type
    #'
    #' @return A base R type, e.g. a list or numeric/character array.
    toSimpleType = function() {
      RosterResponseObject <- list()
      if (!is.null(self$`count`)) {
        RosterResponseObject[["count"]] <-
          self$`count`
      }
      if (!is.null(self$`total`)) {
        RosterResponseObject[["total"]] <-
          self$`total`
      }
      if (!is.null(self$`limit`)) {
        RosterResponseObject[["limit"]] <-
          self$`limit`
      }
      if (!is.null(self$`offset`)) {
        RosterResponseObject[["offset"]] <-
          self$`offset`
      }
      if (!is.null(self$`has_next`)) {
        RosterResponseObject[["has_next"]] <-
          self$`has_next`
      }
      if (!is.null(self$`data`)) {
        RosterResponseObject[["data"]] <-
          lapply(self$`data`, function(x) x$toSimpleType())
      }
      return(RosterResponseObject)
    },

    #' @description
    #' Deserialize JSON string into an instance of RosterResponse
    #'
    #' @param input_json the JSON input
    #' @return the instance of RosterResponse
    fromJSON = function(input_json) {
      ...
      if (!is.null(this_object$`data`)) {
        self$`data` <- ApiClient$new()$deserializeObj(this_object$`data`, "array[RosterPublic]", loadNamespace("chickenstats.api"))
      }
      self
    },
    ...
    #' @return RosterResponse in JSON format
    ...
    #' @description
    #' Validate JSON input with respect to RosterResponse and throw an exception if invalid
    #'
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`count`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for RosterResponse: the required field `count` is missing."))
      }
      ...
      if (!is.null(input_json$`total`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for RosterResponse: the required field `total` is missing."))
      }
      ...
      if (!is.null(input_json$`limit`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for RosterResponse: the required field `limit` is missing."))
      }
      ...
      if (!is.null(input_json$`offset`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for RosterResponse: the required field `offset` is missing."))
      }
      ...
      if (!is.null(input_json$`has_next`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for RosterResponse: the required field `has_next` is missing."))
      }
      ...
      if (!is.null(input_json$`data`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for RosterResponse: the required field `data` is missing."))
      }
    },
    ...
    #'
    #' @return String representation of RosterResponse
    toString = function() {
      self$toJSONString()
    },
...
# RosterResponse$unlock()
#
## Below is an example to define the print function
# RosterResponse$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
