...
#' @description ChangesResponse Class
...
#' @field data  list(\link{ChangesPublic})
#' @importFrom R6 R6Class
#' @importFrom jsonlite fromJSON toJSON
#' @export
ChangesResponse <- R6::R6Class(
  "ChangesResponse",
  public = list(
    `count` = NULL,
    `total` = NULL,
    ...
    #' @description
    #' Initialize a new ChangesResponse class.
    #'
    #' @param count count
    #' @param total total
    #' @param limit limit
    ...
    #'
    #' @return ChangesResponse as a base R list.
    #' @examples
    #' # convert array of ChangesResponse (x) to a data frame
    #' \dontrun{
    #' library(purrr)
    #' library(tibble)
    ...
    #' @description
    #' Convert ChangesResponse to a base R type
    #'
    #' @return A base R type, e.g. a list or numeric/character array.
    toSimpleType = function() {
      ChangesResponseObject <- list()
      if (!is.null(self$`count`)) {
        ChangesResponseObject[["count"]] <-
          self$`count`
      }
      if (!is.null(self$`total`)) {
        ChangesResponseObject[["total"]] <-
          self$`total`
      }
      if (!is.null(self$`limit`)) {
        ChangesResponseObject[["limit"]] <-
          self$`limit`
      }
      if (!is.null(self$`offset`)) {
        ChangesResponseObject[["offset"]] <-
          self$`offset`
      }
      if (!is.null(self$`has_next`)) {
        ChangesResponseObject[["has_next"]] <-
          self$`has_next`
      }
      if (!is.null(self$`data`)) {
        ChangesResponseObject[["data"]] <-
          lapply(self$`data`, function(x) x$toSimpleType())
      }
      return(ChangesResponseObject)
    },

    #' @description
    #' Deserialize JSON string into an instance of ChangesResponse
    #'
    #' @param input_json the JSON input
    #' @return the instance of ChangesResponse
    fromJSON = function(input_json) {
      ...
      if (!is.null(this_object$`data`)) {
        self$`data` <- ApiClient$new()$deserializeObj(this_object$`data`, "array[ChangesPublic]", loadNamespace("chickenstats.api"))
      }
      self
    },
    ...
    #' @return ChangesResponse in JSON format
    ...
    #' @description
    #' Validate JSON input with respect to ChangesResponse and throw an exception if invalid
    #'
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`count`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for ChangesResponse: the required field `count` is missing."))
      }
      ...
      if (!is.null(input_json$`total`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for ChangesResponse: the required field `total` is missing."))
      }
      ...
      if (!is.null(input_json$`limit`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for ChangesResponse: the required field `limit` is missing."))
      }
      ...
      if (!is.null(input_json$`offset`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for ChangesResponse: the required field `offset` is missing."))
      }
      ...
      if (!is.null(input_json$`has_next`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for ChangesResponse: the required field `has_next` is missing."))
      }
      ...
      if (!is.null(input_json$`data`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for ChangesResponse: the required field `data` is missing."))
      }
    },
    ...
    #'
    #' @return String representation of ChangesResponse
    toString = function() {
      self$toJSONString()
    },
...
# ChangesResponse$unlock()
#
## Below is an example to define the print function
# ChangesResponse$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
## Uncomment below to lock the class to prevent modifications to the method or field
# ChangesResponse$lock()
